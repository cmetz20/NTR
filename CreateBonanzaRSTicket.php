...
if (!function_exists('CheckIfBonanzaCustomerExists')) {
	function checkIfBonanzaCustomerExists($user_id, $contact_email){
		global $RS_user_id;
		global $RS_user_url;
		$curl = curl_init();
		if(!$user_id){
...
if (!function_exists('createNewBonanzaCustomer')) {
	function createNewBonanzaCustomer($user_id,$name,$contact_email,$address,$city,$state,$zip){
		global $RS_user_id;
		global $RS_user_url;
		$curl = curl_init();
		curl_setopt_array($curl, array(
...
if (!function_exists('createNewBonanzaTicket')) {
	function createNewBonanzaTicket($order_description){
		global $RS_user_id;
		global $RS_user_url;
		global $RS_ticket_number;
		$curl = curl_init();
		curl_setopt_array($curl, array(
			...
			CURLOPT_POSTFIELDS => array(
				...
				'subject' => "Bonanza - " . $order_description,
...
function createBonanzaRSTicket($name, $user_id, $contact_email, $address, $city, $state, $zip, $country, $order_id, $order_description, $quantity, $date_purchased){
	global $RS_user_id;
	global $RS_user_url;
	global $RS_ticket_number;
	global $temp_description;
	if(!checkIfBonanzaCustomerExists($user_id, $contact_email)){
		...
		createNewBonanzaCustomer($user_id,$name,$contact_email,$address,$city,$state,$zip);
	}
	else{
	...
	while($i <= $quantity){
		if($quantity > 1){
			...
			createNewBonanzaTicket($temp_description);
		}
		else{
			createNewBonanzaTicket($order_description);
		}
		$txt = date('Y-m-d H:i:s') . "- Bonanza Ticket Created for order with quantity = ". $quantity;
		$myfile = file_put_contents('/home/logs/script-logs/get-orders-current-month-log.txt', $txt.PHP_EOL , FILE_APPEND | LOCK_EX);
		$i++;
	}
	updateBonanzaDB($order_id,$RS_ticket_number);
